...
public class ItinerariosController : ControllerBase
{
    ...
    /// }
    /// </remarks>
    [HttpGet("itinerario/{itinerarioId:guid}/mapa")]
    ...
    /// <summary>
    /// Obtém os dados de mapa agregados da linha (ida e volta).
    /// </summary>
    /// <param name="linhaId">Identificador da linha.</param>
    /// <param name="incluirParadas">Define se as paradas serão retornadas no payload.</param>
    /// <param name="cancellationToken">Token de cancelamento da requisição.</param>
    [HttpGet("por-linha/{linhaId:guid}/mapa")]
    [ProducesResponseType(typeof(ItinerarioMapaLinhaDTO), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> BuscarMapaPorLinha(
        Guid linhaId,
        [FromQuery] bool incluirParadas = true,
        CancellationToken cancellationToken = default)
    {
        var resultado = await _service.BuscarMapaPorLinhaAsync(linhaId, incluirParadas, cancellationToken);
        return Ok(resultado);
    }
}
