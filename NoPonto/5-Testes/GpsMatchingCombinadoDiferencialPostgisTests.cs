...
public sealed class GpsMatchingCombinadoDiferencialPostgisTests : IClassFixture<PostgisGpsFixture>
{
    ...
    public async Task QueryCombinada_EquivaleAosDoisCaminhosAtuais_CampoACampo(int numero)
    {
        ...
        var globalAntigo = globalAntigoDto is null
            ...
            : ResultadoBuscaPadrao.Found(globalAntigoDto);
        var anteriorAntigo = await _repo.BuscarEnriquecimentoDoPadraoAsync(
    ...
    public async Task FalhaRealDoComandoCombinado_EhInfrastructureFailureNosDoisRamos()
    {
        ...
        Assert.Equal(StatusBuscaPadrao.InfrastructureFailure, resultado.Global.Status);
        Assert.Equal(StatusBuscaPadrao.InfrastructureFailure, resultado.Anterior.Status);
    ...
    public async Task GlobalB_EProjecaoOperacionalADeOutraLinha_RetornamNoMesmoComando()
    {
        ...
        Assert.Equal(StatusBuscaPadrao.Found, resultado.Global.Status);
        Assert.Equal(_db.R2, resultado.Global.Rota!.PadraoVersaoId);
        Assert.Equal(StatusProjecaoOperacional.Encontrada, resultado.Operacional!.Status);
        Assert.Equal(_db.OutraLinha, resultado.Operacional.Projecao!.PadraoVersaoId);
    ...
    public async Task ProjecaoOperacional_DistingueInelegibilidadeDeFalhaDeInfraestrutura()
    {
        ...
        await using var fonte = NpgsqlDataSource.Create(builder.ConnectionString);
        var repoFalho = new GpsPadraoRepository(
    ...
    {
        var inicial = (await _repo.BuscarEnriquecimentoDoPadraoAsync(
        ...
        Assert.Equal(decisaoAntiga.PadraoVersaoId, decisaoCombinada.PadraoVersaoId);
    ...
    private Cenario CriarCenario(int numero) => numero switch
    {
        ...
        8 => C("bearing oposto", "GPS23", _db.R1, -22.9, -43.2, 270, .4, .6,
            StatusBuscaPadrao.Found, StatusBuscaPadrao.NotEligible),
        ...
        18 => C("global inelegivel anterior encontrado", "X25", _db.X, 0, 0, 135, .78, .85,
            StatusBuscaPadrao.NotEligible, StatusBuscaPadrao.Found),
        19 => C("ambos inelegiveis", "SEM_ROTA", _db.R1, -22.9, -43.2, 90, .4, .6,
            StatusBuscaPadrao.NotEligible, StatusBuscaPadrao.NotEligible),
    ...
    private static Cenario C(string nome, string linha, Guid anteriorId, double lat, double lon,
        ...
        StatusBuscaPadrao global = StatusBuscaPadrao.Found,
        StatusBuscaPadrao anterior = StatusBuscaPadrao.Found) =>
        new(nome, linha, anteriorId, lat, lon, bearing, 250, new(min, max), global, anterior);

    private static void AssertEquivalente(
        ResultadoBuscaPadrao esperado, ResultadoBuscaPadrao atual, string contexto)
    {
    ...
    private static GpsEnriquecimentoService CriarServico(IGpsPadraoRepository repo) => new(
    ...
    private sealed record Cenario(
        ...
        StatusBuscaPadrao StatusAnterior);

    private sealed class RepositorioDePares : IGpsPadraoRepository
    {
        private readonly Queue<EnriquecimentoRotaDto?> _globais;
        private readonly Queue<ResultadoBuscaPadrao> _anteriores;

        public RepositorioDePares(
            EnriquecimentoRotaDto inicial,
            EnriquecimentoRotaDto globalAtual,
            ResultadoBuscaPadrao anteriorAtual)
        {
            _globais = new Queue<EnriquecimentoRotaDto?>([inicial, globalAtual]);
            _anteriores = new Queue<ResultadoBuscaPadrao>([anteriorAtual]);
        }
